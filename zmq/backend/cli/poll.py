...
# we need to fish out a pointer to the "internal" static delegate PollMany in ZPollItems.
zpit = clr.GetClrType(ZeroMQ.ZPollItems)
pmfi = zpit.GetField("PollMany", System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.NonPublic)
PollMany = pmfi.GetValue(None)

def zmq_poll(sockets, timeout):
    socks = List[ZSocket]()
    events = List[ZPollItem]()
    allflags = 0
    for s,f in sockets:
        socks.Add(s)
        events.Add(_zpollitem(f))
        allflags |= int(f)

    #socks.Poll(events, Enum.Parse(ZPoll, str(allflags)), None, TimeSpan.FromMilliseconds(timeout))
    (res, err) = PollMany(socks, events, Enum.Parse(ZPoll, str(allflags)), TimeSpan.FromMilliseconds(timeout))
    return [(s, i.ReadyEvents) for s, i in zip(socks, events)]
